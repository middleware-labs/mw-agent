...
// WithKubeAgentVersion sets the agent version
func WithKubeAgentVersion(v string) KubeAgentOptions {
...
func WithKubeAgentLogger(logger *zap.Logger) KubeAgentOptions {
	return func(h *KubeAgent) {
		h.logger = logger
	}
}

// WithKubeAgentClientset sets the clientset to be used with agent
func WithKubeAgentClientset(clientset kubernetes.Interface) KubeAgentOptions {
	return func(h *KubeAgent) {
		h.clientset = clientset
	}
}

// WithKubeAgentHTTPGetFunc sets the httpGetFunc to be used with agent
func WithKubeAgentHTTPGetFunc(httpGetFunc func(string) (*http.Response, error)) KubeAgentOptions {
	return func(h *KubeAgent) {
		h.httpGetFunc = httpGetFunc
	}
}

// NewKubeAgent returns new agent monitor for Kubernetes with given options.
func NewKubeAgent(cfg BaseConfig, opts ...KubeAgentOptions) (*KubeAgent, error) {
	agent := KubeAgent{
		BaseConfig: cfg,
		version:    "0.0.0",
	}
	...
	if agent.clientset == nil {
		...
		agent.clientset, err = kubernetes.NewForConfig(config)
		if err != nil {
			return nil, err
		}
	}

	if agent.httpGetFunc == nil {
...
func (c *KubeAgent) UpdateConfigMap(ctx context.Context, componentType ComponentType) error {
	...
	resp, err := c.httpGetFunc(baseURL.String())
	if err != nil {
