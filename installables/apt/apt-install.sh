...
sudo wget -O /usr/local/bin/melt-go-agent/apt/pgp-key-$MELT_VERSION.public https://host.middleware.io/public-keys/pgp-key-$MELT_VERSION.public
...
echo "deb [arch=all signed-by=/usr/local/bin/melt-go-agent/apt/pgp-key-$MELT_VERSION.public] https://host.middleware.io/repos/$MELT_VERSION/apt-repo stable main" | sudo tee /etc/apt/sources.list.d/melt-go.list
