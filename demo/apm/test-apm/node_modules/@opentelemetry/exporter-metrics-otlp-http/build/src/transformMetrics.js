...
function toAggregationTemporality(metric) {
    if (metric.descriptor.metricKind === sdk_metrics_base_1.MetricKind.OBSERVABLE_GAUGE) {
        return exporter_trace_otlp_http_1.otlpTypes.opentelemetryProto.metrics.v1.AggregationTemporality
...
function toCollectorMetric(metric, startTime) {
    ...
    if (metric.aggregator.kind === sdk_metrics_base_1.AggregatorKind.SUM ||
        metric.descriptor.metricKind === sdk_metrics_base_1.MetricKind.OBSERVABLE_COUNTER ||
        metric.descriptor.metricKind === sdk_metrics_base_1.MetricKind.OBSERVABLE_UP_DOWN_COUNTER) {
        const result = {
...
function toCollectorResourceMetrics(groupedMetrics, baseAttributes, startTime) {
    return Array.from(groupedMetrics, ([resource, libMetrics]) => {
        return {
            resource: exporter_trace_otlp_http_1.toCollectorResource(resource, baseAttributes),
