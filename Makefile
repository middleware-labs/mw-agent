...
package-kube-config-updater:
	docker build . --platform linux/arm64,linux/amd64 --target prod --build-arg GITHUB_TOKEN=$(GH_TOKEN) --build-arg AGENT_VERSION=${RELEASE_VERSION}  -t ghcr.io/middleware-labs/mw-kube-agent-config-updater:local -f Dockerfiles/DockerfileKubeConfigUpdater
